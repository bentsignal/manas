...
export const sourceGapLabel=gap=>gap.kind==='content_withheld'?'[Passage withheld: sexual content involving a minor]':gap.kind==='meaning_unresolved'?'[Source wording unresolved; translation pending]':SOURCE_GAP_LABEL;
...
export function validateRelease(rows, target, sources, gaps=[]) {
 ...
 for(const gap of gaps){
  ...
  if((gap.kind==='content_withheld'?gap.status!=='withheld':gap.status!=='unresolved')||!['damaged','meaning_unresolved','content_withheld'].includes(gap.kind??'damaged')||!gap.evidence||!gap.source_line_ids?.length||!source||gap.source?.sha256!==source.sha256)throw new Error('Undocumented source gap');
  const touchesSource=after>=0&&before===after+1&&(rows[after].source_id===gap.source_id||rows[before].source_id===gap.source_id);
  if(!touchesSource)throw new Error('Source gap must sit beside an adjacent translated row from its source');
