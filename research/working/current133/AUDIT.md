...
Therefore manuscript folios **1–34 precede the first released printed-witness line**. They are a securely identified untranslated physical segment. Their exact line count is not recoverable from the whole-item total alone and must not be guessed: it requires a diplomatic transcription of those leaves. The secure numeric novelty statement is consequently **at least one untranslated narrative line, exact count unresolved**, across a 34-folio segment.
...
| secure untranslated segment | folios 1–34 | precedes the first released `seytek-2012` row |
| secure unique untranslated lines | at least 1 | exact count unresolved pending transcription |
...
This inventory is **not completion-covered** on present evidence. It is the first audited inventory in this sequence with a securely located physical segment outside the released printed witness. The compact evidence is in `monotonic-crosswalk.json`.
